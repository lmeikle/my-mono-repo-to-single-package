...
describe('Calculate the distance between 2 sets of latitude and longitude coordinates', function() {
  test('Given 2 sets of valid coordinates return the distance', function() {
    let data = validData;
    expect(calculateDistance(data.startLatitude, data.startLongitude, data.endLatitude, data.endLongitude)).toBe(data.expectedResult);
  });

  test('Given invalid coordinates return NaN', function() {
    let data = invalidData;
    expect(Number.isNaN(calculateDistance(data.startLatitude, data.startLongitude, data.endLatitude, data.endLongitude))).toBe(true);
  });
});
